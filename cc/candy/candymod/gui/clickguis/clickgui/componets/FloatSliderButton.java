...
public class FloatSliderButton extends Component
{
    ...
    public FloatSliderButton(final Module module, final Setting<Float> setting, final int x, final int width, final int height) {
        this.module = module;
    ...
    public void mouseClickMove(final int mouseX, final int mouseY, final int clickedMouseButton) {
        if (this.x - 10 < mouseX && this.x + this.width + 10 > mouseX && clickedMouseButton == 0 && this.changing) {
            ...
            this.setting.setValue(Math.round(((this.setting.maxValue - this.setting.minValue) * this.value + this.setting.minValue) * 10.0f) / 10.0f);
        }
    }
